$( document ).ready(function() {
	$(".form__input-prix").on("change paste keyup", function() {
		...
		total = prix_1 + prix_2 + prix_3 + prix_4 + prix_5 + prix_6;
		base_98_25 = (total*0.9825).toFixed(2);
		cotisation_maladies_vieillesse = (total*0.004).toFixed(2);
		csg = (total*0.9825*0.092).toFixed(2);
		crds = (total*0.9825*0.005).toFixed(2);
		cfpc = (total*0.9825*0.0035).toFixed(2);
		total_precompte = ( +(cotisation_maladies_vieillesse) + +(csg) + +(crds) + +(cfpc)).toFixed(2);
		...
		$("input[name$='base_98-25']").val(base_98_25);
		...
		$("input[name$='total-artiste']").val(total_artiste);
